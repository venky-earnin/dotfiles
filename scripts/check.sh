...
echo "==> shell syntax"
for file in scripts/*.sh home/bin/agent-* home/.claude/hooks/*.sh; do
  bash -n "$file"
done
...
if command -v shellcheck >/dev/null 2>&1; then
  echo "==> shellcheck"
  shellcheck scripts/*.sh home/bin/agent-* home/.claude/hooks/*.sh
else
...
if command -v shfmt >/dev/null 2>&1; then
  echo "==> shfmt"
  shfmt -i 2 -ci -d scripts/*.sh home/bin/agent-* home/.claude/hooks/*.sh
else
